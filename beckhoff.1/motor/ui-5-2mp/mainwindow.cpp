 ...
 *---------------------------------------------------------------------------*/
  ...
  QString macro12,pmac,nmac,amac,dmac,mac1,mac2,macch,m12mac;
  _haveM=0;
  if(pm){
    macro.append(pm);
    _haveM=1;
  ...
  m12mac.append(mac1).append(",").append(mac2);
  _mc1=_mc2=0; _bkh=0;
  ...
  macro12.append(",").append(amac).append(",").append(dmac);
  macro12.append(",").append(macch);
...
void MainWindow::showM1Control(){
  if(!_mc1){
    ...
    _mc1=new MotorCtrl( "Motor X Control",this);
    prof.releaseProfile();
  }
 if(_mc1) _mc1->show();
}
void MainWindow::showM2Control(){
  if(!_mc2){
    ...
    _mc2=new MotorCtrl( "Motor Y Control",this);
    prof.releaseProfile();
  }
 if(_mc2) _mc2->show();
}
